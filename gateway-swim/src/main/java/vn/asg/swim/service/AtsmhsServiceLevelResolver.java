...
public class AtsmhsServiceLevelResolver {
    ...
    /**
     * Phân giải cấp độ dịch vụ ATSMHS.
     *
    ...
    /**
     * Chế độ Recipients-based:
     * Nếu tất cả người nhận hỗ trợ extended ATSMHS → EXTENDED, ngược lại → BASIC.
     */
    private String resolveByRecipients(String recipients) {
        ...
        if (extendedCapableAddresses.isBlank()) {
            log.warn("ATSMHS: ATSMHS_EXTENDED_CAPABLE_ADDRESSES chưa được cấu hình - recipients-based → BASIC");
            return BASIC;
        }
    ...
    /**
     * Kiểm tra tính hợp lệ của nội dung theo cấp độ dịch vụ.
     * Chế độ BASIC không thể xử lý nội dung nhị phân (binary).
     *
