...
import _int.icao.iwxxm._2023_1.AIRMETEvolvingConditionCollectionPropertyType;
import _int.icao.iwxxm._2023_1.AIRMETEvolvingConditionPropertyType;
import _int.icao.iwxxm._2023_1.AIRMETEvolvingConditionType;
...
import _int.icao.iwxxm._2023_1.AeronauticalAreaWeatherPhenomenonType;
 ...
 * @author ThanhNk
 */
public class AIRMETReverter extends Reverter<AIRMETType, AIRMET> {
    ...
    @Override
    public AIRMET convert(String content) throws IWXXMParsingException {
        AIRMETType speci = null;
        try {
            speci = revert(content, AIRMETType.class);
    ...
    public String convertToString(String content) throws IWXXMParsingException {
        AIRMET speci = convert(content);
        return speci == null ? null : speci.toString();
    }

    @Override
    public AIRMET convert(AIRMETType type) throws IWXXMParsingException {
        if (type == null) {
            return null;
        }

        AIRMET airmet = new AIRMET();
        this.parse(type, airmet);
        return airmet;
    }

    public void parse(AIRMETType objectType, AIRMET airmet) throws IWXXMParsingException {
        if (objectType == null) {
            return;
        }
...
//        sigmet.setCreateDate(setCreateDate(objectType)); 
        airmet.setCreateDate(this.dateTimeFormat.format(issiuedDate));
        //REPORT TYPE 
//        sigmet.setTypeofReport(getTypeofReport());
        //getLocationIndicatorOfFir
        airmet.setLocationIndicatorOfFir(getLocationIndicatorOfFir(objectType.getIssuingAirTrafficServicesUnit(), false));

        airmet.setNumberIdentification(getNumberIdentification(objectType.getSequenceNumber().getValue()));//        /Validity period  (M) 

        airmet.setValidityPeriod(getValidPeriod(objectType.getValidPeriod()));
        //Location indicator of  MWO:
        airmet.setLocationIndicatorMWO(getLocationIndicatorOfFir(objectType.getOriginatingMeteorologicalWatchOffice(), true));//getLocationIndicatorMWO(objectType.getOriginatingMeteorologicalWatchOffice());    
        this.locationIndicatorMWO = airmet.getLocationIndicatorMWO();
        //Time from 
        airmet.setValidFrom(getFrom(objectType.getValidPeriod()));
        //Time to 
        airmet.setValidTo(getTo(objectType.getValidPeriod()));
        //Name of the FIR/CTA or aircraft identification (M)
        airmet.setFir(getFir(objectType.getIssuingAirTrafficServicesRegion()));
        // Cancel 
...
//        }
        //Phenomenon :
        airmet.setPhenomenon(getPhenomenon(objectType.getPhenomenon()));
        //Date of OBS, Localtion; Levell; Movement; Intensity 
        airmet.setDateofObs(getDateOfObs(objectType.getAnalysis()));//da ok can kt. moi duoc OBS  thieu thoi gian AT 1210Z   

        proAssociationRoleType(objectType.getAnalysis(), airmet);
        //OBS/FCST :      
//        airmet.setObsfcst(getObsfcst(objectType.getForecastPositionAnalysis()));
        //RAWW
        airmet.setRaw(getRaw(objectType.getDescription()));
        //Cancellation : 
        airmet.setCancellation(getCancellation(objectType));
//        return sigmet;
    }
...
//            buffer.append(strType);
    ...
    private String getPhenomenon(AeronauticalAreaWeatherPhenomenonType objectType) {
    ...
    private void proAssociationRoleType(AIRMETEvolvingConditionCollectionPropertyType object, SIGMET sigmet) {
        if (object == null) {
            return;
        }
//        object.getAIRMETEvolvingConditionCollection()
...
//        dateOfObs.append(getDateOfObs(object)); 
//        List<SIGMETEvolvingConditionPropertyType> conditionPropertyTypes = obj.getValue().getMember();
        List<AIRMETEvolvingConditionPropertyType> conditionPropertyTypes = object.getAIRMETEvolvingConditionCollection().getMember();
//        dateOfObs.append(getDateOfObs(obj));
        for (AIRMETEvolvingConditionPropertyType it : conditionPropertyTypes) {
...
///List<AIRMETEvolvingConditionPropertyType>
    //private String getDateOfObs(JAXBElement<SIGMETEvolvingConditionCollectionType> obj) {

    private String getDateOfObs(AIRMETEvolvingConditionCollectionPropertyType obj) {
//        String no="NO VA EXP";
        if (obj == null) {
            return "";
        }
        StringBuilder buffer = new StringBuilder();
        if (!obj.getAIRMETEvolvingConditionCollection().isSetTimeIndicator()) {
            return "";
        }
        String timeIndicatorType = "";
        if (obj.getAIRMETEvolvingConditionCollection().getTimeIndicator().equals(TimeIndicatorType.OBSERVATION)) {
            timeIndicatorType = "OBS";
        } else if (obj.getAIRMETEvolvingConditionCollection().getTimeIndicator().equals(TimeIndicatorType.FORECAST)) {
        ...
        if (!obj.getAIRMETEvolvingConditionCollection().isSetPhenomenonTime()) {
            return "";
        }
        AbstractTimeObjectPropertyType abstractTimeObjectPropertyType = obj.getAIRMETEvolvingConditionCollection().getPhenomenonTime();
    ...
    private String getLocaltion(AIRMETEvolvingConditionPropertyType it) {
        StringBuilder buffer = new StringBuilder();
        if (!it.isSetAIRMETEvolvingCondition()) {
            return "";
        }
        AIRMETEvolvingConditionType conditionType = it.getAIRMETEvolvingCondition();
    ...
    private String getLevel(AIRMETEvolvingConditionPropertyType obj) {
        if (obj == null) {
            return "";
        }
        StringBuilder buffer = new StringBuilder();
        if (!obj.isSetAIRMETEvolvingCondition()) {
            return "";
        }
        AIRMETEvolvingConditionType conditionType = obj.getAIRMETEvolvingCondition();
        if (!conditionType.isSetGeometry()) {
            return "";
        }
        AirspaceVolumePropertyType airspaceVolumePropertyType = conditionType.getGeometry();
...
//        }
    ...
    private String getMovement(AIRMETEvolvingConditionPropertyType obj) {
        if (obj == null) {
            return "";
        }
        StringBuilder buffer = new StringBuilder();
        ...
        if (conditionType.isSetDirectionOfMotion()) {
            ang = obj.getAIRMETEvolvingCondition().getDirectionOfMotion();
        }
        if (ang != null) {
    ...
    private String getIntensity(AIRMETEvolvingConditionPropertyType obj) {
        if (obj == null) {
            return "";
        }
        StringBuilder buffer = new StringBuilder();
        ...
        AIRMETExpectedIntensityChangeType changeType = conditionType.getIntensityChange();
        buffer.append(getIntensityChangeType(changeType)).append(" ");
        return buffer.toString();
    }

    private String getIntensityChangeType(AIRMETExpectedIntensityChangeType type) {
        String out = "";
        switch (type) {
            case INTENSIFY:
                out = "INTSF";
...
//RAWW
    ...
    private String getCancellation(AIRMETType obj) {
        ...
        if (obj.isSetCancelledReportSequenceNumber()) {
            ...
            buffer.append("CNL SIGMET " + obj.getCancelledReportSequenceNumber());
        }
        if (periodType.isSetBeginPosition()) {
