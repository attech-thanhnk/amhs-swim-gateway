...
public class SIGMETTacMessage extends TacMessageImpl {
    ...
    public void parseMessage() throws SIGMETParsingException {
        ...
        Matcher matcher = SigmetParsingRegexp.sigmetNILHeader.matcher(tac);
        if (matcher.find()) {
            ...
            this.setMessageIssueDateTime(dtNow);
        ...
        matcher = getHeaderPattern().matcher(tac);
        if (matcher.find()) {
            ...
            try {
                this.setMessageIssueDateTime(IWXXM31Helpers.parseDateTimeToken(matcher.group("dateFrom")));
                ...
                throw new SIGMETParsingException("Check date and time in VALID sections. [" + CoreUtil.truncateString(firCode, lastIndex));
            }
            ...
            String remain = tac.toString().trim();
            if (remain.startsWith("NIL")) {
                this.nil = true;
                return;
            }
        ...
        try {
            ...
            throw new IllegalArgumentException(ex.getMessage() + " [" + CoreUtil.truncateString(tac.toString(), 100) + "]", ex);
        }
        ...
        if (matcherMov.find()) {
            ...
            if (isMoving) {
                ...
                if (speed != null && units != null) {
                    ...
                    movSection.setMovingSpeed(Integer.parseInt(speed));
                }
            }
    ...
    /**
     * Extract location with multilines example: N OF LINE N5100 E03520 - N5017 E04200 AND S OF LINE N5400 E03150 - N5440 E04400
     *
     * @param tac
     * @param location
     * @return 
    ...
    protected StringBuffer fillMultiLineLocation(StringBuffer tac, SigmetHorizontalPhenomenonLocation location) {
        ...
        while (matcherDirLine.find()) {
            ...
            CoordPoint startPoint = new CoordPoint(RUMB_UNITS.valueOf(latitudeStart), Integer.parseInt(latStartDeg),
                    Integer.parseInt(latStartMin),
                    RUMB_UNITS.valueOf(longitudeStart), Integer.parseInt(longStartDeg), Integer.parseInt(longStartMin));

            CoordPoint endPoint = new CoordPoint(RUMB_UNITS.valueOf(latitudeEnd), Integer.parseInt(latEndDeg),
                    Integer.parseInt(latEndMin),
                    RUMB_UNITS.valueOf(longitudeEnd), Integer.parseInt(longEndDeg), Integer.parseInt(longEndMin));
            Line line = new Line();
    ...
    public void setCancelSigmetDateTimeTo(DateTime cancelSigmetDateTimeTo) {
     ...
     public String getIdentifier() {
        return String.format("SIGMET_%s_%s.xml",
                this.getIcaoCode(),
     ...
     public String createMessageID() {
        return String.format("A_%s%s%s%s%s%s_C_%s_%s.xml",
                ...
                31,
                this.getIcaoCode(),
                this.getMessageIssueDateTime().toString("ddHHmm"),
                "",
                this.getIcaoCode(),
                this.getMessageIssueDateTime().toString("yyyyMMddHHmmss"));
    }
