...
public class METARTacMessage extends TacMessageImpl {
    ...
    private boolean snowclosed = false;

    private LinkedList<METARRVRSection> rvrSections = new LinkedList<>();
    private LinkedList<METARRunwayStateSection> runwayStateSections = new LinkedList<>();
    private LinkedList<String> windShearSections = new LinkedList<>();

    private LinkedList<METARBecomingSection> becomingSections = new LinkedList<>();
    private LinkedList<METARTempoSection> tempoSections = new LinkedList<>();
    private LinkedList<METARRemarkSection> remarkSections = new LinkedList<>();

    private LinkedList<MetarForecastTimeSection> timedSections = new LinkedList<>();
    private MERTARSeaStateSection seaStateSection;

    public METARTacMessage(String initialTac) {
        super(normalize(initialTac));
    }
    ...
    /**
     * Performs parsing message applying regexps step-by-step, truncating processed groups from message /* @throws METARParsingException
     *
     * @throws org.gamc.spmi.iwxxmConverter.metarconverter.METARParsingException
     */
    @Override
    public void parseMessage() throws METARParsingException {
        ...
        if (matcher.lookingAt()) {
            ...
            String tacInString = tac.toString().trim();
            
            if (tacInString.startsWith("NIL")) {
                this.nil = true;
                return;
            }
            
            if (tacInString.startsWith("AUTO")) {
                this.auto = true;
            }
            ...
            // String header = tac.length() < 20 ?  tac.substring(0) : tac.substring(20);
            throw new METARParsingException("Mandatory header section is missed in " + getTacStartToken() + " [ " + CoreUtil.truncateString(tac.toString(), 20) + "]");
        }
        ...
        // process Runway Visual Range - RVR 
        ...
        // SNOCLO
        matcher = MetarParsingRegexp.metarSNOCLO.matcher(tac);
        if (matcher.find()) {
            this.setSnowclosed(true);
            int startIndex = matcher.start();
            lastIndex = matcher.end();
            tac.delete(startIndex, lastIndex);
        }
        
        // Parsing runway state
        ...
        while (matcher.find()) {
            ...
            } else if (sRWDesignator.equalsIgnoreCase("99")) {
                rwsSection.setInApplicate(false);
            }
        ...
        matcher = MetarParsingRegexp.metarSeaSection.matcher(tac);
        if (matcher.find()) {
            this.seaStateSection = new MERTARSeaStateSection(tac.substring(matcher.start(), matcher.end()));
            tac.delete(matcher.start(), matcher.end());
        }
    ...
    /**
     * Method for parsing the common weather section that may exists in METAR itself or its trend sections
     */
    ...
    /**
     * Seeks for the BECMG, TEMPO sections in initial TAC message If found puts them into inner collections and removes from the message to avoid duplicate groups in visibility, precipitation etc..
     */
    ...
    /**
     * Seeks for the RMK section in initial TAC message. If found puts it into inner collections and removes from the message to avoid duplicate groups in visibility, precipitation etc..
     *
     */
    ...
    public String getIdentifier() {
        return String.format("METAR_%s_%s.xml",
                this.getIcaoCode(),
    ...
    /**
     * @param snowclosed the snowclosed to set
     */
    public void setSnowclosed(boolean snowclosed) {
        this.snowclosed = snowclosed;
    }

    /**
     * @return the seaStateSection
     */
    public MERTARSeaStateSection getSeaStateSection() {
        return seaStateSection;
    }
