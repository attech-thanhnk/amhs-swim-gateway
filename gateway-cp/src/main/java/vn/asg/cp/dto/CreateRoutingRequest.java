...
public class CreateRoutingRequest {
        ...
        @Schema(description = "AMQP topic to subscribe (required for IN direction)", example = "ats/met/metar")
        private String receiveTopic;

        @Schema(description = "Optional content filter (for IN direction)", example = "METAR")
        private String messageFilter;

        @Schema(description = "Space-separated AFTN addresses (required for IN direction)", example = "VVHHZTZX VVTSZDYX")
        private String recipients;

        @Schema(description = "AFTN originator address (required for IN direction)", example = "VVHHZQZX")
        private String originator;

        // ========== OUTBOUND DIRECTION (AMHS → SWIM) ==========
        @Schema(description = "Message type to detect (required for OUT direction)", example = "METAR")
        private String messageType;

        @Schema(description = "AMQP topic to publish (required for OUT direction)", example = "ats/met/metar")
        private String sendTopic;

        // ========== COMMON PROPERTIES ==========
        @Schema(description = "Priority (0-255, lower number = higher priority)", example = "100")
